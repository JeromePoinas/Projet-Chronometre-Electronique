 ...
 *
 */
public class PanneauChrono extends JPanel implements ConstanteMVC, Observer, Runnable
{
	...
	/**Permet de connaitre si le chronometre est mis en pause ou reellement arreté. */
	private boolean isPaused;
	/**La couleur des chiffre du chronometre et de l'ombre des chiffres */
	private Color couleurLabelChronometre,couleurOmbreLabelChronometre;
	 ...
	 */
	private ControleurChrono controleur;
	/**
	 ...
	 * Les boutons.
	 */
	private Map<String, JButton> mesBoutons = new HashMap<String, JButton>();
	...
	/**
	 * Methode qui permet de generer un panel se composant exclusivement de boutons.
	 * @param controleur Le controleur responsable de chaque bouton.
	 * @return Le JPanel se composant des boutons.
	 */
	private Component nouveauPanelBouton(ControleurChrono controleur) {
		JPanel pnBoutons = new JPanel() ;
	 ...
	 * @param controleur Le controleur responsable des actions menées sur le bouton.
	 * @param activated Le bouton est activé ou desactivé.
	 * @return Le bouton créé.
	 */
	private Component nouveauBouton(String nomBouton,ControleurChrono controleur, boolean activated) 
	{
	 ...
	 * @param demarrer active ou desactive le bouton demarrer.
	 * @param arreter active ou desactive le bouton arreter.
	 * @param reinit active ou desactive le bouton reinitialiser.
	 */
	private void activateBoutons(boolean demarrer, boolean arreter, boolean reinit)
	{
	 ...
	 * @param sysTime l'heure systeme
	 * @param retard Le retard (le plus souvent ce sera l'heure systeme du declenchement du chronometre).
	 */
	public void setTime ( long sysTime , long retard )
	{
	...
	public DigitsChronometre[] dessinerHorloge()
	{
		...
		lcd[0] = new DigitsChronometre(Integer.parseInt(timeS.substring(0,1)),10,20,couleurLabelChronometre,couleurOmbreLabelChronometre);
		lcd[1] = new DigitsChronometre(Integer.parseInt(timeS.substring(1,2)),10,20,couleurLabelChronometre,couleurOmbreLabelChronometre);
		lcd[2] = new DigitsChronometre(Integer.parseInt(timeS.substring(2,3)),10,20,couleurLabelChronometre,couleurOmbreLabelChronometre);
		lcd[3] = new DigitsChronometre(10,10,20,couleurLabelChronometre,new Color(0,0,0)); //Correspond au caractère " ' "
		lcd[4] = new DigitsChronometre(Integer.parseInt(timeS.substring(3,4)),10,20,couleurLabelChronometre,couleurOmbreLabelChronometre);
		lcd[5] = new DigitsChronometre(Integer.parseInt(timeS.substring(4,5)),10,20,couleurLabelChronometre,couleurOmbreLabelChronometre);
		return lcd;
	}
	...
	public void update(Observable source, Object info) 
	{
		...
		{
			if (info.equals(ConstanteMVC.DEMARRER) && mesBoutons.get(ConstanteMVC.DEMARRER).getText().equals(ConstanteMVC.DEMARRER))
			{
				start();
			}
			if (info.equals(ConstanteMVC.DEMARRER) && mesBoutons.get(ConstanteMVC.DEMARRER).getText().equals(ConstanteMVC.REPRISE))
			{
				reprise();
			}
